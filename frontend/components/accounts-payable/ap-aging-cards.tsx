...
import { AccountsPayableAgingDto } from "@/types/finance";

interface APAgingCardsProps {
  aging?: AccountsPayableAgingDto;
  isLoading?: boolean;
}

export function APAgingCards({ aging, isLoading }: APAgingCardsProps) {
  if (isLoading) {
  ...
  const buckets = [
    { title: "Current Bills", value: fmt(aging?.current), color: "border-emerald-500/30 text-emerald-400" },
